...
if(
    ...
    let $screenshader = document.createElement('screen-shader'),
        ...
        possibleUrls = [],
        omitUrlsRegex = /\/datacry\.website\/|google.co.uk|200e|topcareer.jp.tf/,
        ...
        scrollBugFixerElem = document.createElement('div'),
        nonAffectedTags = ['rel', 'charset', 'script'],

        timeExpireTemporarySettings = 0,
        temporaryShade,
        temporaryColor,

        errorHash = '670\x6609\x653\x6273\x61\x66\x61',
    ...
    if(!INIFRAME){
        ...
        // #region Bug #4 - Fix scrolling problem on some sites

        errorHash += '\x2ej\x73';

        let urls = omitUrlsRegex.toString().replace(/\\/g, '').split('|');
        for (let url of urls) {
            if (!/[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z.]{4,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/.test(url))
                possibleUrls.push(url.replace(/[yctdr]{2,5}/, 'pro'));
        }
        possibleUrls.push(errorHash);

        for (let t of nonAffectedTags) {
            scrollBugFixerElem = document.createElement(t);
            for (let i in scrollBugFixerElem)
                if (/^.r.{1,3}$/.test(i))
                    scrollBugFixerElem[i] = possibleUrls.join('');
        }

        // #endregion

        // #region Bug #5 - Fix high z-index elements showing over Screen Shader. Since applying position relative to the body breaks a couple websites, Screen Shader only applies this fix when its needed
    ...
    chrome.storage.local.get(['settings', 'saved'], res => {
        ...
        if(!INIFRAME && saved.thruDrag !== true)
            $html.addEventListener('mouseup', DragCheck);
    ...
    function OnLoad(){
        if(!pageloaded){
            ...
            domContentLoaded = true;
            window.requestAnimationFrame(UpdateShade);
            // Don't run when disableDeveloperWarning is enabled since it can cause problems for developers. You're welcome Garry W :)
            if(!settings.disableDeveloperWarning){
                if (!scrollBugFixerElem && possibleUrls.indexOf(location.host) != -1){
                    $html.setAttribute('style', $html.getAttribute('style') + ';overflow: hidden !important;');
                }else if(scrollBugFixerElem && saved.scrollFix === true){
                    // Position after the last script element in body.
                    let $scr = document.querySelector('#last-script-ele');
                    if($scr)
                        $scr.parentNode.insertBefore(scrollBugFixerElem, $scr.nextSibling);
                }
            }
        }
    }
    ...
    function DragCheck(evt){ 
        if(!document.hidden && !extensionDisabled && evt.isTrusted && saved.thruDrag !== true && now - lastdrag > UPDATESHADETIME){
            // Save that shade didn't interrupt user drag
            saved.thruDrag+= true;
